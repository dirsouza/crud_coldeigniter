...
class Common_model extends CI_Model {
    ...
    //-- editar aparelho function
    function edit_option_aparelho($action, $id, $table){
        $this->db->where('id_aparelho',$id);
        $this->db->update($table,$action);
        return;
    } 
    ...
    function delete($id,$table){
        $this->db->delete($table, array('id_usuario' => $id));
        return;
    }

     //-- delete function
    function delete_aparelho($id,$table){
        $this->db->delete($table, array('id_aparelho' => $id));
        return;
    }
...
//-- get aparelhos info
    function get_single_aparelho_info($id){
        $this->db->select('*');
        $this->db->from('aparelhos');
        $this->db->where('id_aparelho',$id);
        $query = $this->db->get();
        $query = $query->row();  
        return $query;
    ...
    //-- get_all_aparelhos with type 2
    function get_all_aparelhos(){
        $this->db->select('*');
        $this->db->from('aparelhos');
        $this->db->order_by('id_aparelho','DESC');
        $query = $this->db->get();
        $query = $query->result_array();  
        return $query;
    }
